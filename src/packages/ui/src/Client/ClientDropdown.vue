...
function updateSearchValue(event) {
	...
	if (newInput === ' ') {
		...
		if (highlightedClientId) {
			const highlightedClient = props.clients.find((client) => client.id === highlightedClientId);
...
function moveHighlightUp() {
	if (highlightedItem.value) {
		const currentHightlightedIndex = filteredClients.value.indexOf(highlightedItem.value);
		if (currentHightlightedIndex === 0) {
			highlightedItemId.value = filteredClients.value[filteredClients.value.length - 1].id;
		} else {
			highlightedItemId.value = filteredClients.value[currentHightlightedIndex - 1].id;
		}
	}
}
		...
		<template #content>
			...
			<div ref="dropdownViewport" class="w-60 max-h-60 overflow-y-scroll">
				...
				<div
					...
					:value="client.id"
					:class="{
						'bg-card-background-active': client.id === highlightedItemId,
					}"
