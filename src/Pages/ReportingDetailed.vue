		...
		<div class="py-2.5 w-full border-b border-default-background-separator">
			<MainContainer class="sm:flex space-y-4 sm:space-y-0 justify-between">
				<div class="flex flex-wrap items-center space-y-2 sm:space-y-0 space-x-4">
					...
					<MemberMultiselectDropdown v-model="selectedMembers" @submit="updateFilteredTimeEntries">
						<template #trigger>
					...
					<ProjectMultiselectDropdown v-model="selectedProjects" @submit="updateFilteredTimeEntries">
						<template #trigger>
							<ReportingFilterBadge
					...
					<TaskMultiselectDropdown v-model="selectedTasks" @submit="updateFilteredTimeEntries">
						<template #trigger>
							<ReportingFilterBadge
					...
					<ClientMultiselectDropdown v-model="selectedClients" @submit="updateFilteredTimeEntries">
						<template #trigger>
							<ReportingFilterBadge title="Clients" :icon="FolderIcon"></ReportingFilterBadge>
						</template>
					</ClientMultiselectDropdown>
					<TagDropdown v-model="selectedTags" :create-tag :tags="tags" @submit="updateFilteredTimeEntries">
						<template #trigger>
							<ReportingFilterBadge
			...
			<PaginationList v-slot="{ items }" class="flex items-center space-x-1 relative">
				<div class="pr-2 flex items-center space-x-1 border-r border-border-primary mr-1">
					...
					<PaginationPrev class="mr-4 navigation-item">
						<ChevronLeftIcon class="w-4 text-text-tertiary hover:text-text-primary"> </ChevronLeftIcon>
					</PaginationPrev>
				</div>
				<template v-for="(page, index) in items">
					<PaginationListItem v-if="page.type === 'page'" :key="index" class="pagination-item" :value="page.value">
						{{ page.value }}
					</PaginationListItem>
				...
				<div class="!ml-2 pl-2 flex items-center space-x-1 border-l border-border-primary">
					<PaginationNext class="navigation-item">
						<ChevronRightIcon class="w-4 text-text-tertiary hover:text-text-primary"></ChevronRightIcon>
					</PaginationNext>
					<PaginationLast class="navigation-item">
						<ChevronDoubleRightIcon class="w-4 text-text-tertiary hover:text-text-primary"></ChevronDoubleRightIcon>
					</PaginationLast>
				</div>
			</PaginationList>
		</PaginationRoot>
