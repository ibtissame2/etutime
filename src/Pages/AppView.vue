<template>
	<div>
		...
		<div v-if="!showEnvironments" class="questionnaire-container">
			...
			<div class="main-panel">
				...
				<div class="title-container">
					<h1 class="main-title">Questionnaire sur la concentration et l'organisation du travail</h1>
				</div>
				...
				<div v-if="!termine" class="questionnaire-content">
					...
					<div class="progress-container">
						<div :style="{ width: pourcentage + '%' }" class="progress-bar"></div>
					</div>

					<!-- Compteur de questions avec design pastel -->
					<div class="question-counter">
						<div class="question-badge">Question {{ currentQuestionNumber }}</div>
						<div class="question-total">{{ currentQuestionNumber }} sur {{ totalQuestions }}</div>
					</div>
					...
					<div v-if="currentQuestion.type === 'ouvert'" class="open-response">
						<div class="textarea-container">
							<textarea v-model="reponseOuverte" placeholder="Votre réponse..." class="open-textarea"></textarea>
						</div>
						<div class="submit-container">
							<button @click="soumettreReponseOuverte" class="submit-button">Soumettre</button>
						</div>
					</div>
				</div>
				...
				<div v-if="termine" class="completion-screen">
					<div class="completion-content">
						...
						<div class="concentration-result">
							<p>
								Votre niveau de concentration est :
							...
							<p v-if="scoreDetails">
								Organisation: {{ Math.round(scoreDetails.organisation * 100) }}% | Concentration:
								{{ Math.round(scoreDetails.concentration * 100) }}% | Motivation:
								{{ Math.round(scoreDetails.motivation * 100) }}%
							</p>
						</div>

						<div class="button-container">
							<button @click="navigateToTimeTable" class="start-study-button">Commencer l'étude</button>
						</div>
					</div>
				</div>
			</div>
		...
		<div v-if="showEnvironments" class="environments-view">
			<EmploiUpload :niveau="concentrationLevel" ref="emploiUpload" />
			<EmploiCree :niveau="concentrationLevel" @enregistrer="handleEnregistrement" />
		</div>
	</div>
</template>
...
export default {
		...
		return {
			...
			questionsParCategorie: [
				...
				[
					...
					{
						texte:
							"Si ton niveau de concentration pouvait être évalué en temps réel, comment voudrais-tu que l'application adapte ton environnement de travail ?",
						type: 'ouvert',
						options: [],
					},
				],
	...
	methods: {
		...
		async analyserReponses() {
			...
			try {
				...
				if (response?.niveau) {
					...
					await this.sauvegarderQuestionnaire(response.niveau, Math.round((response.score_moyen || 0) * 100));
				}
			} catch (error) {
		...
		async recupererDernierQuestionnaire() {
			try {
				const credentials = getCredentials();
				const response = await fetch('questionnaires/get_questionnaire', { credentials });
