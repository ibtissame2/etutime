 ...
 * Original file: /npm/@editorjs/header@2.8.7/dist/header.umd.js
 *
	 ...
	 */ class v {
		...
		normalizeData(e) {
			...
			return (
				this.isHeaderData(e) &&
					((t.text = e.text || ''),
					e.level !== void 0 && !isNaN(parseInt(e.level.toString())) && (t.level = parseInt(e.level.toString()))),
				t
			);
		}
		render() {
		...
		get levels() {
			const e = [
				{ number: 1, tag: 'H1', svg: s },
				{ number: 2, tag: 'H2', svg: o },
				{ number: 3, tag: 'H3', svg: h },
				{ number: 4, tag: 'H4', svg: d },
				{ number: 5, tag: 'H5', svg: u },
				{ number: 6, tag: 'H6', svg: g },
			];
		...
		onPaste(e) {
			...
			if ('data' in t) {
				...
				this._settings.levels &&
					(r = this._settings.levels.reduce((a, l) => (Math.abs(l - r) < Math.abs(a - r) ? l : a))),
					(this.data = { level: r, text: i.innerHTML });
			}
		}
