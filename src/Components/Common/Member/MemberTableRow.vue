...
export default {
	...
	setup(props) {
		...
		return {
			...
			showMakeMemberPlaceholderModal,
			removeMember,
			invitePlaceholder,
...
<template>
	<TableRow>
		...
		<div class="whitespace-nowrap px-3 py-4 text-sm text-muted">
			{{ member.billable_rate ? formatCents(member.billable_rate, getOrganizationCurrencyString()) : '--' }}
		</div>
		...
		>
			<SecondaryButton
				v-if="member.is_placeholder === true && canInvitePlaceholderMembers() && userHasValidMailAddress"
				size="small"
