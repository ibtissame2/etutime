...
async function deleteReport() {
	await handleApiRequestNotifications(() => deleteReportMutation.mutateAsync(props.report.id), 'Success', 'Error');
}
</script>
			...
			<div v-if="report.shareable_link" class="space-x-2 flex items-center">
				...
				>
					<ArrowTopRightOnSquareIcon
						class="w-4 text-text-tertiary hover:text-text-secondary transition"
					></ArrowTopRightOnSquareIcon>
				</button>
			</div>
			<span v-else> -- </span>
