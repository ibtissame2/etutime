...
import { daysLeftInTrial, isBillingActivated, isBlocked, isFreePlan, isInTrial } from '@/utils/billing';
...
const hideBlockedBanner = useSessionStorage('showBlockedBanner-' + getCurrentOrganizationId(), false);
const showBlockedBanner = computed(() => isBlocked() && !hideBlockedBanner.value);
const hideFreeUpgradeBanner = useSessionStorage('showFreeUpgradeBanner-' + getCurrentOrganizationId(), false);
const showFreeUpgradeBanner = computed(
	() => isFreePlan() && !isBlocked() && !hideFreeUpgradeBanner.value && !showBlackFridayBanner.value
);
const hideBlackFridayBanner = useSessionStorage('hideBlackFridayBanner-' + getCurrentOrganizationId(), false);
...
<template>
	<div v-if="showBlackFridayBanner" class="bg-tertiary text-xs lg:text-sm pb-1 pt-2 border-b border-border-secondary">
			...
			<div class="flex items-center space-x-1.5">
				...
				<div class="flex-1 space-x-1">
					<span class="font-medium">
						<strong>BLACK FRIDAY SALE!</strong> Use the code <strong>BLACKFRIDAY</strong> at checkout and get
						<strong>30% off</strong> the solidtime yearly plan.
					</span>
				</div>
			</div>
			<div class="flex items-center space-x-2">
				<router-link v-if="canManageBilling()" href="/billing">
					<div
	...
	<div v-if="showTrialBanner" class="bg-accent-600/50 text-xs lg:text-sm py-0.5 border-b border-border-secondary">
		<MainContainer class="flex items-center justify-between">
			<div class="flex items-center space-x-1.5">
				<!-- <CheckBadgeIcon class="w-4 text-text-primary/50"></CheckBadgeIcon> -->
				<div class="flex-1 space-x-1">
					<span class="font-medium"> Your trial expires in {{ daysLeftInTrial() }} days. </span>
					<span class="hidden md:inline">
						To continue using all features & support the development of solidtime, please upgrade your plan.
					</span>
				</div>
			</div>
			<div class="flex items-center space-x-2">
				<router-link v-if="canManageBilling()" to="/billing">
					<div
						class="text-text-primary font-semibold uppercase text-xs flex space-x-1 items-center hover:bg-white/10 rounded-lg px-2 py-1.5"
					>
						<span>Upgrade now</span>
	...
	<div v-if="showBlockedBanner" class="bg-red-600/50 text-xs lg:text-sm py-0.5 border-b border-border-secondary">
		<MainContainer class="flex items-center justify-between">
			<div class="flex items-center space-x-1.5">
				...
				<div class="flex-1 space-x-1">
					...
					<span class="hidden md:inline">
						Please upgrade to a premium plan or remove all users except the owner to unblock your organization.
					</span>
				</div>
			</div>
			<div class="flex items-center space-x-2">
	...
	<div v-if="showFreeUpgradeBanner" class="bg-tertiary text-xs lg:text-sm py-0.5 border-b border-border-secondary">
		<MainContainer class="flex items-center justify-between">
			<div class="flex items-center space-x-1.5">
				<!-- <XCircleIcon class="w-4 text-text-primary/50"></XCircleIcon> -->
				<div class="flex-1 space-x-1">
					...
					<span class="hidden md:inline">
						To unlock all premium features & support the development of solidtime, please upgrade your plan.</span
					>
				</div>
			</div>
			<div class="flex items-center space-x-2">
