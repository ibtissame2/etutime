...
import { useSelectEvents } from '@/Components/src/utils/select';

const currentTimeEntry = defineModel('currentTimeEntry', { required: true });
const liveTimer = defineModel('liveTimer', { required: true });

const currentTimeEntryDescriptionInput = ref(null);
...
function setAndStartTimer(timeEntry) {
...
function setCurrentTimeEntry(timeEntry) {
...
function onToggleButtonPress(newState) {
	if (newState) {
		emit('startTimer');
...
const filteredRecentlyTrackedTimeEntries = computed(() => {
	...
	return nonDuplicateTimeEntries
		.filter((item) => {
			return item.description?.toLowerCase()?.includes(tempDescription.value?.toLowerCase()?.trim() || '');
		})
		.slice(0, 5);
});
...
const highlightedDropdownEntryId = ref(null);

useSelectEvents(filteredRecentlyTrackedTimeEntries, highlightedDropdownEntryId, (item) => item.id, showDropdown);
</script>
		...
		>
			<div class="flex flex-1 items-center pr-6 relative">
				...
				>
					...
					>
						<div class="text-text-tertiary text-xs font-semibold border-b border-border-tertiary px-2 py-1.5">
